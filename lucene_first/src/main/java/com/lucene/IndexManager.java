...
//        p1: 域的名称  p2: 值  p3:是否存储原内容 create web page.txt
    ...
    @Test
    public void testWriterOne() throws Exception{
//        准备一个存储索引文件的路径
        FSDirectory directory = FSDirectory.open(new File("D:\\class61\\index_repo"));
        Analyzer analyzer = new IKAnalyzer();
        IndexWriterConfig conf = new IndexWriterConfig(Version.LATEST,analyzer);
        IndexWriter indexWriter = new IndexWriter(directory,conf);
            Document document = new Document();
//        p1: 域的名称  p2: 值  p3:是否存储原内容 create web page.txt
        TextField filename = new TextField("filename", "welcome china laomei laohei apache solr project.txt", Field.Store.YES);
        filename.setBoost(10);  //设置打分 默认值是1
        document.add(filename);

            document.add(new TextField("filecontent","welcome china laomei  apache solr project", Field.Store.YES));
            document.add(new StringField("filepath","d:\\welcome china laomei  apache solr project", Field.Store.YES));
...
//        Query query = new TermQuery(new Term("filename","spring is a project"));  //根据term查询 域名：值
...
//        filename:spring filename:project

//        QueryParser queryParser = new MultiFieldQueryParser(Version.LATEST,new String[]{"filename","filecontent"},new IKAnalyzer());
//        Query query = queryParser.parse("spring is a project");
//        filename:spring filename:project filecontent:spring filecontent:project


//        Query query = new MatchAllDocsQuery(); //查询所有
//        Query query = NumericRangeQuery.newLongRange("filesize",100L,957L,true,false);

//        BooleanQuery query = new BooleanQuery();
//        Query query1 = new TermQuery(new Term("filename","apache"));  //根据term查询 域名：值
//        Query query2 = NumericRangeQuery.newLongRange("filesize",800L,957L,true,false);
